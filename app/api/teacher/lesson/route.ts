...
export async function POST(request: Request) {
  try {
    ...
    const payload = (await request.json()) as { gradeBand?: string; page?: number; active?: boolean; completed?: boolean };
    ...
    const completed = !active && payload.completed === true;
    const lessonState = active ? 1 : completed ? 2 : 0;
    const phase = active ? "active" : completed ? "completed" : "waiting";
    ...
    await db.batch([
      ...
      `).bind(roomCode, payload.gradeBand, page, sourceSlide, lessonState),
      ...(active ? [db.prepare(`
      ...
      `).bind(`teacher:${roomCode}`, roomCode, JSON.stringify({ gradeBand: payload.gradeBand, page, sourceSlide, active, phase })),
    ]);

    return Response.json({ roomCode, gradeBand: payload.gradeBand, page, sourceSlide, active, phase, updatedAt: new Date().toISOString() });
  } catch (error) {
